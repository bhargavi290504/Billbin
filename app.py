...
from notify_utils import send_notification  # Ensure this exists and doesn't crash
...
CLIENT_SECRETS_FILE = "credentials.json"
SCOPES = ['https://www.googleapis.com/auth/calendar.readonly']
REDIRECT_URI = "https://billbin.onrender.com/callback"  # Use your live domain
...
def dashboard():
    ...
    if request.method == 'POST':
        try:
            if 'limit' in request.form:
                user.limit += int(request.form['limit'])
                user.min_balance = int(request.form['min_balance'])
                db.session.commit()
